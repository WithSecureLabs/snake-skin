...
export default {
  ...
  computed: {
    ...mapGetters({
      storePages: 'files/storePages',
      storePageData: 'files/storePageData'
    }),

    ...mapState({
      files: state => state.files.files,
      running: state => state.files.running,

      // TEMP
      page: state => state.files.currentPage,
      rows: state => state.files.rows
    })
  },
  ...
  methods: {
    ...mapActions({
      ...
      getFilteredFiles: 'files/getFilteredFiles',
      initStore: 'files/initStore'
    }),

    ...mapMutations({
      setStoreCurrentPage: 'files/setStoreCurrentPage',
      setStoreRows: 'files/setStoreRows'
    }),
